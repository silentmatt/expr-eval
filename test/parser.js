...
describe('Parser', function () {
  [
    ...
    describe(tcase.name, function () {
      ...
      it('should accept variables starting with G', function () {
        assert.strictEqual(parser.parse('GRAVITY * 2').evaluate({ GRAVITY: 9.8 }), 19.6);
      });
      ...
      it('should parse constants', function () {
        assert.strictEqual(parser.evaluate('G'), 6.67300e-11);
        assert.strictEqual(parser.evaluate('PHI'), (1 + Math.sqrt(5)) / 2);
        assert.strictEqual(parser.evaluate('LN2'), Math.LN2);
        assert.strictEqual(parser.evaluate('C'), 299792458);
      })
